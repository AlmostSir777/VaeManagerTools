...
#import "MultyRecordTool.h"
#import "lame.h"
#import "ALiProgressHUD.h"
@interface MultyRecordTool () <AVAudioPlayerDelegate, AVAudioRecorderDelegate>
...
@implementation MultyRecordTool
...
- (void)updateImage {
    if(self.timeBlock)
    {
        self.timeBlock([MultyRecordTool sharedSoundRecorder].recorder.currentTime);
    }
    ...
    if([MultyRecordTool sharedSoundRecorder].recorder.currentTime >60.f)
    {
        [ALiProgressHUD showImage:K_IMG(@"GTH" ) status:@"录制时间过长,已断开"];
        [[MultyRecordTool sharedSoundRecorder]stopRecorder];
